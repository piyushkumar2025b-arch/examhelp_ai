...
try:
    ...
    print("Groq Request failed:", e.reason if hasattr(e, 'reason') else e)
    if hasattr(e, 'read'):
...
req_gemini = urllib.request.Request(url_gemini, data=data_gemini, headers={"Content-Type": "application/json"})
try:
    ...
    print("Gemini Request failed:", e.reason if hasattr(e, 'reason') else e)
    if hasattr(e, 'read'):
        print("Body:", e.read().decode("utf-8"))
